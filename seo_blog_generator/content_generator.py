...
===CONTENT_END===""".format(
    ...
    def _call_gemini_api(self, prompt, language="English", keywords_context=""):
...
- 优先使用长尾关键词和语义相关的词汇"""
            ...
            # 构建完整的提示词
            full_prompt = f"""{self.system_prompt}{keywords_section}
...
Please generate a comprehensive, SEO-optimized article about this topic following the guidelines above."""
    ...
    def generate_article(self, topic, language="English", keywords_context=""):
        """生成文章"""
        try:
            # 调用 API 生成内容
            raw_content = self._call_gemini_api(topic, language, keywords_context)
